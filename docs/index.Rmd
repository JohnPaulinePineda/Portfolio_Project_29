---
title: 'R : Exploring Performance Evaluation Metrics for Survival Prediction'
...
# **1. Table of Contents**
|
| This document implements a comparison of evaluation metrics for assessing survival model performance using various helpful packages in <mark style="background-color: #CCECFF">**R**</mark>.    
|
| Survival analysis aims to study the relationship between independent covariates and survival time and event outcomes. Evaluation metrics in survival analysis are used to ascertain the concordance between the predicted risk score for each observation and the actual survival time, considering the censoring condition. The metrics applied in this study (mostly contained in the <mark style="background-color: #CCECFF">**SurvMetrics**</mark> package) attempt to evaluate the performance of survival model predictions.
|
##  1.1 Sample Data
|
...
|
| **[A]** Numeric variables which demonstrated differential relationships with the <span style="color: #FF0000">ttodead</span> and <span style="color: #FF0000">died</span> survival time and event status variables, based on correlation with survival time, include:
...
|
| **[C]** To simplify the subsequent computations, only three numeric variables (which demonstrated the most intuitive trend line with survival time and consistency of distribution) and three factor variables (which demonstrated the lowest Log-Rank Test p-value and sufficient separation in the estimated Kaplan-Meier survival curve) were evaluated during survival modelling which include:
...
|
| **[D]** The median split-sample cross-validated CIN based on 5 iterations  are summarized as follows: :
|      **[D.1]** RSF = 0.63738
|      **[D.2]** CPH = 0.66629
|      **[D.3]** The range of cross-validated CIN metrics was relatively higher for the CPH model as compared to RSF model.
|
...
(CPH_Model_CIN_CV <- median(SplitValidation_CIN[SplitValidation_CIN$Model=="CPH",1]))
(RSF_Model_CIN_CV <- median(SplitValidation_CIN[SplitValidation_CIN$Model=="RSF",1]))
...
|
| [BSC](https://journals.ametsoc.org/view/journals/mwre/78/1/1520-0493_1950_078_0001_vofeit_2_0_co_2.xml) is the mean square difference between the true classes and predicted probabilities. Serving as a a form of cost function, it is considered a proper score which means that it takes its minimal value only when the predicted probabilities match the empirical probabilities.
|
...
| **[C]** The CPH model does not contain any hyperparameter.
|
| **[D]** The average split-sample cross-validated BSC based on 5 iterations are summarized as follows: :
|      **[D.1]** RSF = 0.14325
|      **[D.2]** CPH = 0.13853
|      **[D.3]** The range of cross-validated BSC metrics was relatively higher for the CPH model as compared to RSF model.
|
...
###  1.5.3 Integrated Brier Score (IBSC)
|
| [IBSC](https://onlinelibrary.wiley.com/doi/10.1002/sim.1621) is the integrated weighted squared distance between the estimated survival function and the empirical survival function. The inverse probability censoring weighting(IPCW) is used to adjust for censoring.
|
| **[A]** The IBSC metric for the survival time range was calculated for both the formulated Random Survival Forest (RSF) and Cox Proportional Hazards (CPH) models using the <mark style="background-color: #CCECFF">**SurvMetrics**</mark>, <mark style="background-color: #CCECFF">**randomForestSRC**</mark> and <mark style="background-color: #CCECFF">**survival**</mark> packages.  
|
| **[B]** The RSF model contains 2 hyperparameters:
|      **[B.1]** <span style="color: #FF0000">ntree</span> = number of trees held constant at a value of 500.
|      **[B.2]** <span style="color: #FF0000">nsplit</span> = number of random splits for splitting a variable held constant at a value of 3.
...
| **[C]** The CPH model does not contain any hyperparameter.
|
| **[D]** The average split-sample cross-validated IBSC based on 5 iterations are summarized as follows: :
|      **[D.1]** RSF = 0.xxxx
|      **[D.2]** CPH = 0.xxxx
|      **[D.3]** The range of cross-validated BSC metrics was relatively higher for the CPH model as compared to RSF model.
|
...
# # for all variables
# ##################################
# MA_IBSC <- peakVO2.Source[,names(peakVO2.Source) %in% c("ttodead","died","interval","insulin")]
# 
# ##################################
# # Initializing data containers
# # and random seed
...
#  # Formulating the train and test sets
#  ##################################
#  MA_IBSC_Index = createFolds(1:nrow(MA_IBSC), 2)
#  MA_IBSC_Train = MA_IBSC[MA_IBSC_Index[[1]],]
#  MA_IBSC_Test = MA_IBSC[MA_IBSC_Index[[2]],]
# 
#  ##################################
#  # Formulating the Random Survival Forest model
#  # and generating model predictions
...
#                    nsplit = 3,
#                    ntree = 500)
#  RSF_Model_PredObject <- predict(RSF_Model, MA_IBSC_Test)
# 
#  RSF_Model_Pred <- RSF_Model_PredObject$survival
# 
#  ##################################
...
# 
#  ##################################
#  # Calculating the Integrated Brier Scores
#  ##################################
#  Median_Index <- median(1:length(Ordered_Unique_Death_Time))
#  Survival_Object <- Surv(MA_IBSC_Test$ttodead, MA_IBSC_Test$died)
# 
#  RSF_Model_IBSC[i] <- IBS(Survival_Object,
#                           sp_matrix = RSF_Model_Pred,
#                           Ordered_Unique_Death_Time)
# 
#  CPH_Model_IBSC[i] <- IBS(Survival_Object,
...
# ##################################
# # Consolidating the repeated
# # split-sample validated Integrated Brier Scores
# ##################################
# (SplitValidation_IBSC = data.frame('IBSC' = c(CPH_Model_IBSC, RSF_Model_IBSC),
#                                  'Model' = c(rep('CPH', 5), rep('RSF', 5)),
#                                  'Metric' = c(rep('IBSC', 10))))
# 
# (CPH_Model_IBSC_CV <- mean(SplitValidation_IBSC[SplitValidation_IBSC$Model=="CPH",1]))
# (RSF_Model_IBSC_CV <- mean(SplitValidation_IBSC[SplitValidation_IBSC$Model=="RSF",1]))
#
# bwplot(IBSC ~ Model | Metric,
#        data = SplitValidation_IBSC,
#        xlab = "Survival Model",
#        ylab = "Integrated Brier Score")

```

###  1.5.4 Integrated Absolute Error (IAE)
|
| [IAE](https://link.springer.com/article/10.1007/s10985-016-9372-1) is a type of continuous-time approach to continuous-time identification based on least-absolute errors.
|
| **[A]** The IBSC metric for the survival time range was calculated for both the formulated Random Survival Forest (RSF) and Cox Proportional Hazards (CPH) models using the <mark style="background-color: #CCECFF">**SurvMetrics**</mark>, <mark style="background-color: #CCECFF">**randomForestSRC**</mark> and <mark style="background-color: #CCECFF">**survival**</mark> packages.  
|
| **[B]** The RSF model contains 2 hyperparameters:
...
| **[C]** The CPH model does not contain any hyperparameter.
|
| **[D]** The average split-sample cross-validated IAE based on 5 iterations are summarized as follows: :
|      **[D.1]** RSF = 0.xxxx
|      **[D.2]** CPH = 0.xxxx
|      **[D.3]** The range of cross-validated BSC metrics was relatively higher for the CPH model as compared to RSF model.
|
...
###  1.5.5 Integrated Square Error (ISE)
|
| [ISE](https://link.springer.com/article/10.1007/s11424-021-9307-0) is a type of continuous-time approach to continuous-time identification based on least-squares errors.
|
...
|
| **[A]** Details
|      **[A.1]** Details : Details (<mark style="background-color: #CCECFF">**Details**</mark> package)
|             **[A.1.1]** Details = 0.xxxxx
|             **[A.1.2]** Details = 0.xxxxx
|
